USE [SmartWorking]
DELETE FROM [Smartworking].[dbo].[DeliveryNotes]
GO
DELETE FROM [Smartworking].[dbo].[MaterialStocks]
GO
DELETE FROM [Smartworking].[dbo].[Orders]
GO
DELETE FROM [Smartworking].[dbo].[RecipeComponents]
GO
DELETE FROM [Smartworking].[dbo].[Materials]
GO
DELETE FROM [Smartworking].[dbo].[Cars]
GO
DELETE FROM [Smartworking].[dbo].[ClientBuildings]
GO
DELETE FROM [Smartworking].[dbo].[Clients]
GO
DELETE FROM [Smartworking].[dbo].[Contractors]
GO
DELETE FROM [Smartworking].[dbo].[Buildings]
GO
DELETE FROM [Smartworking].[dbo].[Drivers]
GO
DELETE FROM [Smartworking].[dbo].[Recipes]
GO